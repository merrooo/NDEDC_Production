...
const FULL_COLUMNS = [
  ...
  "Name of connected KIOSK",
  "Code of Feeder",
  "Name of connected Feeder",
  "SIM_S_Num",
  "BadgeNumber",
  "Manufacturer",
  "Model",
...
async function loadData() {
  showLoading();
  try {
    const snap = await get(ref(db, "MDM_Smart"));
    if (snap.exists()) {
...
function updateStats() {
  ...
  els.forEach((id) => {
    ...
    if (el) el.innerText = currentResults.length;
  });
  const rowLabel = document.getElementById("rowCountLabel");
  if (rowLabel)
    rowLabel.innerHTML = `<i class="fas fa-database"></i> إجمالي العدادات: ${currentResults.length}`;
}
...
function renderTable() {
  ...
  header.innerHTML =
    `<th style="position:sticky; right:0; background:#3b82f6;">الإجراءات</th>` +
...
window.saveRow = async (id, btn) => {
  ...
  try {
    await update(ref(db, `MDM_Smart/${id}`), updates);
...
window.deleteRow = async (id) => {
  ...
  if (res.isConfirmed) {
    await remove(ref(db, `MDM_Smart/${id}`));
...
function extractValue(row, standardColumn) {
  ...
  for (const name of names) {
    ...
    if (val !== undefined && val !== null && val !== "")
      return processValue(val, standardColumn);
  }
  return "";
}
...
document.getElementById("viewAllBtn")?.addEventListener("click", () => {
  ...
  document
    ...
    .forEach((p) => p.classList.remove("active"));
  document
    .querySelector('.nav-btn[data-page="search"]')
...
document.querySelectorAll(".tab-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    document
      ...
      .forEach((b) => b.classList.remove("active"));
    document
      .querySelectorAll("#searchPage .tab-content")
      .forEach((t) => t.classList.remove("active"));
    btn.classList.add("active");
    document
      .getElementById(btn.dataset.tab + "TabContent")
      .classList.add("active");
  });
});
  ...
  processBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (meterDataMap.size === 0) {
        Swal.fire("تنبيه", "لا توجد أرقام عدادات صالحة", "warning");
        hideLoading();
        return;
      }
      const existingSnap = await get(ref(db, "MDM_Smart"));
      ...
      for (const [meterId, newData] of meterDataMap) {
        ...
        if (hasNew || !existing[key]) updates[`MDM_Smart/${key}`] = merged;
      }
      if (Object.keys(updates).length > 0) {
        ...
        Swal.fire({
          ...
          html: `تمت معالجة ${meterDataMap.size} عداد`,
        });
        await loadData();
      } else {
        Swal.fire("مكتمل", "جميع البيانات محدثة", "info");
      }
    } catch (e) {
      Swal.fire("خطأ", "فشل المعالجة", "error");
    }
    hideLoading();
  });
}
    ...
    if (e.target.files.length > 0) {
      ...
      if (list)
        list.innerHTML = Array.from(e.target.files)
          .map((f) => `<li><i class="fas fa-file-excel"></i> ${f.name}</li>`)
          .join("");
...
document.getElementById("applyFilterBtn")?.addEventListener("click", () => {
  const meterVal = cleanVal(
    document.getElementById("searchMeter")?.value || "",
  ).toLowerCase();
  const simVal = cleanVal(
  ...
  const region = cleanVal(
    document.getElementById("searchGenRegion")?.value || "",
  ).toLowerCase();
  const branch = cleanVal(
    document.getElementById("searchRegion")?.value || "",
  ).toLowerCase();
  const typeVal = cleanVal(
  ...
  currentResults = cachedAllData.filter((item) => {
    ...
    return (
      (!meterVal || mNum.includes(meterVal)) &&
      (!simVal || sNum.includes(simVal)) &&
      (!formattedRise || rDate === formattedRise) &&
      (!contractDate || cDate === contractDate) &&
      (!region || reg.includes(region)) &&
      (!branch || br.includes(branch)) &&
      (!typeVal || typ.includes(typeVal))
    );
  });
  currentPage = 1;
  renderTable();
...
document.getElementById("clearFiltersBtn")?.addEventListener("click", () => {
  document
    .querySelectorAll("#searchPage input, #searchPage select")
    .forEach((i) => (i.value = ""));
  currentResults = [...cachedAllData];
  currentPage = 1;
  renderTable();
...
document.getElementById("nextPageBtn")?.addEventListener("click", () => {
  if (currentPage < Math.ceil(currentResults.length / ROWS_PER_PAGE)) {
    currentPage++;
    renderTable();
  }
});
...
document.getElementById("exportBtn")?.addEventListener("click", () => {
  if (currentResults.length === 0)
    return Swal.fire("تنبيه", "لا توجد بيانات", "warning");
  const exportData = currentResults.map((item) => {
    const obj = {};
    FULL_COLUMNS.forEach((col) => (obj[col] = item[col] || ""));
    return obj;
  });
  ...
  XLSX.writeFile(
    wb,
    `smart_meters_${new Date().toISOString().split("T")[0]}.xlsx`,
  );
...
async function loadDashboardData() {
  try {
    ...
    if (snap.exists()) {
      const dataObj = snap.val();
      allData = Object.keys(dataObj).map((key) => ({
        ...dataObj[key],
        firebase_id: key,
      }));
    } else {
...
function updateDashboardStats() {
  document.getElementById("totalMeters").textContent = formatNumber(
    allData.length,
  );
  const weekRange = getCurrentWeekRange();
  const weekMeters = allData.filter((item) => {
    const d = parseDate(item["تاريخ التعاقد"] || item.installDate);
    return d && d >= weekRange.start && d <= weekRange.end;
  }).length;
  document.getElementById("weekMeters").textContent = formatNumber(weekMeters);
  const branches = new Set(
    allData.map((item) => item.فرع || item.discoBranch).filter(Boolean),
  );
  document.getElementById("totalBranches").textContent = formatNumber(
    branches.size,
  );
  const firstDate = allData.reduce((e, i) => {
    const d = parseDate(i["تاريخ التعاقد"] || i.installDate);
    if (!d) return e;
    return e ? (d < e ? d : e) : d;
  }, null);
  let avgDaily = 0;
...
function updateRecentActivity() {
  ...
  const recent = [...allData]
    .filter((i) => i["تاريخ التعاقد"])
    .sort((a, b) => {
      const da = parseDate(a["تاريخ التعاقد"]);
      const db = parseDate(b["تاريخ التعاقد"]);
      return db - da;
    })
    .slice(0, 10);
  if (recent.length === 0) {
    container.innerHTML =
      '<div style="text-align:center; padding:30px;">لا توجد بيانات</div>';
    return;
  }
  container.innerHTML = recent
    .map(
      (item) =>
        `<div class="activity-item"><div class="activity-icon"><i class="fas fa-microchip"></i></div><div class="activity-content"><h4>${item["رقم العداد"] || "غير معروف"}</h4><p>${item.facDesc || "بدون وصف"}</p><div class="activity-details"><span><i class="fas fa-building"></i> ${item.فرع || "-"}</span><span><i class="fas fa-tag"></i> ${item.TYPE || "-"}</span><span><i class="fas fa-calendar"></i> ${item["تاريخ التعاقد"] || "-"}</span></div></div></div>`,
    )
    .join("");
}
...
document.getElementById("printDateSearch") &&
  (document.getElementById("printDateSearch").textContent = dateStr);
document.getElementById("RiseDate") &&
  (document.getElementById("RiseDate").value =
